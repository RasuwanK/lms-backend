   ...
   public function update($id, UpdatePortalUserRequest $request)
    {
        ...
        if (!$request->accepts('application/json')) {
            return ResponseHelper::invalidMedia('Invalid media type. Only JSON is accepted.');
        }
        
        // Only allow PATCH requests
        if (!$request->isMethod('patch')) {
            return ResponseHelper::methodInvalid('Method not allowed. Only PATCH is accepted.');
        }
        ...
        try {
            ...
            if (isset($usersData[0]) && is_array($usersData[0])) {
                ...
                if (!empty($errors)) {
                    return ResponseHelper::error('Some users could not be updated', 207);
                }
            ...
            return ResponseHelper::invalid($e->errors(), 'Validation failed');
        } catch (QueryException $qe) {
            return ResponseHelper::serverError('Database error: ' . $qe->getMessage());
        } catch (Exception $e) {
